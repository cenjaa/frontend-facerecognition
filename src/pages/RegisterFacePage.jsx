...
export default function RegisterFacePage() {
  ...
  useEffect(() => {
    const createUser = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/create_user`, {
          method: 'POST',
  ...
  const uploadDataset = async (blobs) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/dataset/${userId}`, { method: 'POST', headers: { 'X-API-Key': API_KEY }, body: fd });
    ...
    const captureFrame = async () => {
      ...
      try {
        ...
        if (blob) {
          ...
          const res = await fetch(`${API_BASE}/api/infer`, { method: 'POST', headers: { 'X-API-Key': API_KEY }, body: fd });
          const data = await res.json();
